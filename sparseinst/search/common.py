...
class Conv(nn.Module):
    ...
    # ch_in, ch_out, kernel, dilation, stride, padding, groups
    def __init__(self, c1, c2, k=1, d=1, s=1, p=None, g=1, act=True):
        ...
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(
            k, p, d), dilation=d, groups=g, bias=False)
...
class SepConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, d=1, s=1, p=None, g=1, act=True):
        ...
        if isinstance(d, list):
            d = d[0]
        self.dwconv = nn.Conv2d(c1, c1, k, s, autopad(
            k, p, d), dilation=d, groups=c1, bias=False)
        self.pwconv = nn.Conv2d(c1, c2, 1, 1, padding=0,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, k, d=1, shortcut=True, g=1, e=0.5, separable=False):
...
class Conv_search(nn.Module):
    ...
    def __init__(self, c1, c2, kd=[(1, 1), (3, 1), (5, 1), (3, 2)], s=1, p=None, g=1, act=True, gumbel_op=False):
        ...
        for ks in range(len(kd)):
            kernel_size = kd[ks][0]
            dilation = kd[ks][1]
            tmp = nn.Sequential(nn.Conv2d(c1, c2, kernel_size, s, autopad(
                kernel_size, p, dilation), dilation=dilation, groups=g, bias=False), nn.BatchNorm2d(c2))
            self.m.append(tmp)
        self.act = nn.SiLU() if act is True else (
            act if isinstance(act, nn.Module) else nn.Identity())
#        self._alphas = torch.autograd.Variable(1e-3*torch.randn(len(k)), requires_grad=True)
        self.register_buffer('alphas', torch.autograd.Variable(
            1e-3*torch.randn(len(kd)), requires_grad=True))

    def forward(self, x):
        if self.gumbel_op:
            alphas = gumbel_softmax(F.log_softmax(
                self.alphas, dim=-1), hard=True)
...
class Conv_search_merge(nn.Module):
    ...
    def __init__(self, c1, c2, kd=[(1, 1), (3, 1), (5, 1), (3, 2)], candidate_e=[1.], s=1, p=None, g=1, act=True, gumbel_channel=False, with_bn=True, inside_alphas_channel=True, bias=False):
        ...
        for (k, d) in kd:
            if d == 0 and c1 != c2:
                raise (ValueError("d=0 means skip-connection, but c1 != c2."))
            if d == 0 or k == 0:
                continue
            tmp = (k-1)*d + 1
            if tmp > ks:
        ...
        self.act = nn.SiLU() if act is True else (
            act if isinstance(act, nn.Module) else nn.Identity())
        if gumbel_channel:
            self.bn = nn.ModuleList([nn.BatchNorm2d(int(c2*e)) for e in candidate_e]
                                    ) if with_bn else nn.ModuleList([nn.Identity() for _ in candidate_e])
        else:
...
#        self._alphas = torch.autograd.Variable(1e-3*torch.randn(len(k)), requires_grad=True)
        ...
        if len(candidate_e) > 1 and inside_alphas_channel:
            self.register_buffer('alphas_channel', torch.autograd.Variable(
                1e-3*torch.randn(len(candidate_e)), requires_grad=True))
        else:
    ...
    def get_merge_kernel(self, w_base, alphas):
        ...
        for i, alpha in enumerate(alphas):
            ...
            w[:, :, start:end:d, start:end:d] = w_base[:,
    ...
    def forward(self, x):
        ...
        if len(self.candidate_e) > 1:
            ...
            channel_mask = torch.zeros(
                [Cout], dtype=merge_kernel.dtype, device=merge_kernel.device)
            if self.gumbel_channel:
                alphas_channel = gumbel_softmax(F.log_softmax(
                    self.alphas_channel, dim=-1), hard=True)  # alphas_channel is one-hot vector
                for idx, (e, a_e) in enumerate(zip(self.candidate_e, alphas_channel)):
                    if a_e > 0.:
                        bn = self.bn[idx]
                        if bias is not None:
                            bias = bias[:int(self.cout*e)]
                        merge_kernel = merge_kernel[:int(
                            self.cout*e), :Cin, :, :] * a_e
                        break
            else:
                channel_idx = torch.arange(
                    0, Cout, dtype=merge_kernel.dtype, device=merge_kernel.device).long()
      #          channel_idx = torch.sort(merge_kernel.view(Cout,-1).sum(dim=-1), descending=True)[1]
                alphas_channel = nn.functional.softmax(
                    self.alphas_channel, dim=-1)
        ...
        if Cin != merge_kernel.size(1):
            merge_kernel = merge_kernel[:, :Cin, :, :]
        out = torch.nn.functional.conv2d(
            x, merge_kernel, stride=self.stride, padding=self.padding, dilation=1, groups=self.group)
        ...
        if bias is None:
            ...
            return self.act(bn(out)) + bias.view(1, -1, 1, 1)
...
#        self._alphas = torch.autograd.Variable(1e-3*torch.randn(len(k)), requires_grad=True)
    ...
    def forward(self, x):
        ...
        if Cin != merge_kernel.size(0):
            merge_kernel = merge_kernel[:Cin, :, :, :]
        out = torch.nn.functional.conv2d(
            x, merge_kernel, stride=self.stride, padding=self.padding, dilation=1, groups=Cin)
        ...
        if len(self.candidate_e) > 1:
            ...
            if self.gumbel_channel:
                ...
                channel_mask = torch.zeros(
                    [Cout], dtype=pweight.dtype, device=pweight.device)
                channel_idx = torch.arange(
                    0, Cout, dtype=pweight.dtype, device=pweight.device).long()
      #          channel_idx = torch.sort(pweight.view(Cout,-1).sum(dim=-1), descending=True)[1]
        ...
        if Cin != pweight.size(1):
            pweight = pweight[:, :Cin, :, :]
        out = torch.nn.functional.conv2d(
            out, pweight, stride=1, padding=0, dilation=1, groups=self.group)
...
class Bottleneck_search(nn.Module):
    # Standard bottleneck
    # ch_in, ch_out, shortcut, groups, expansion
    def __init__(self, c1, c2, kd=[(3, 1), (5, 1), (3, 2)], shortcut=True, g=1, e=0.5, gumbel_op=False):
...
class Bottleneck_search_merge(nn.Module):
    # Standard bottleneck
    # ch_in, ch_out, shortcut, groups, expansion
    def __init__(self, c1, c2, kd=[(3, 1), (5, 1), (3, 2)], candidate_e=[1.], shortcut=True, g=1, e=0.5, gumbel_channel=False, separable=False):
...
#        self.cv1 = Conv(c1, c_, k=1, d=1, s=1)
        self.cv1 = Conv_search_merge(c1, c_max, kd=[(
            1, 1)], candidate_e=candidate_e, s=1, gumbel_channel=gumbel_channel)
        if separable:
            my_conv = SepConv_search_merge
        else:
            my_conv = Conv_search_merge
        self.cv2 = my_conv(c_max, c2, kd, candidate_e=[
                           1.], s=1, g=g, gumbel_channel=gumbel_channel)
        self.add = shortcut and c1 == c2
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, k=3, d=1, shortcut=True, g=1, e=0.5, e_bottleneck=1., separable=False):
        ...
        if isinstance(d, int):
            ...
            ds = d
        if isinstance(e_bottleneck, float):
            es = [e_bottleneck for _ in range(n)]
        else:
            es = e_bottleneck
        assert len(ks) >= n
        assert len(ds) >= n
        ...
        c1out = int(c2[0]*e)
        c2out = int(c2[-1]*e)  # hidden channels

        self.cv1 = Conv(c1, c1out, k=1, d=1, s=1)
        m_list = []
        cin = c1out
        for i in range(n):
            m_list.append(Bottleneck(
                cin, int(c2[i]*e), ks[i], ds[i], shortcut, g, e=es[i], separable=separable))
            cin = int(c2[i]*e)
...
class C3_search(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, kd=[(3, 1), (5, 1), (3, 2)], shortcut=True, g=1, e=0.5, gumbel_op=False):
        ...
        self.m = nn.Sequential(
            *[Bottleneck_search(c_, c_, kd, shortcut, g, e=1.0, gumbel_op=gumbel_op) for _ in range(n)])
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
...
class C3_search_merge(nn.Module):
    # CSP Bottleneck with 3 convolutions
    # ch_in, ch_out, number, shortcut, groups, expansion
    def __init__(self, c1, c2, n=1, kd=[(3, 1), (5, 1), (3, 2)], candidate_e=[1.], shortcut=True, g=1, e=0.5, search_c2=None, gumbel_channel=False, separable=False):
        ...
        if self.search_c2:
            ...
            c_ = int(c2 * e)  # hidden channels
            self.cv1 = Conv_search_merge(c1, c_, kd=[(
                1, 1)], candidate_e=self.search_c2, s=1, gumbel_channel=gumbel_channel, inside_alphas_channel=False)
            self.cv2 = Conv_search_merge(c1, c_, kd=[(
                1, 1)], candidate_e=self.search_c2, s=1, gumbel_channel=gumbel_channel, inside_alphas_channel=False)
            if gumbel_channel:
                self.cv3 = nn.ModuleList([Conv_search_merge(c_, c2, kd=[(1, 1)], candidate_e=self.search_c2, s=1, gumbel_channel=gumbel_channel,
                                         act=False, with_bn=False, inside_alphas_channel=False) for _ in range(2)])  # act=FReLU(c2)
                self.cv3_bn = nn.ModuleList(
                    [nn.BatchNorm2d(int(c2*e)) for e in self.search_c2])
                self.cv3_act = nn.SiLU()
            else:
                self.cv3 = Conv_search_merge(2 * c_, c2, kd=[(1, 1)], candidate_e=self.search_c2,
                                             s=1, gumbel_channel=gumbel_channel, inside_alphas_channel=False)  # act=FReLU(c2)
            self.register_buffer('alphas_channel', torch.autograd.Variable(
                1e-3*torch.randn(len(self.search_c2)), requires_grad=True))
        else:
            c_ = int(c2 * e)  # hidden channels
            self.cv1 = Conv(c1, c_, k=1, d=1, s=1)
            self.cv2 = Conv(c1, c_, k=1, d=1, s=1)
            self.cv3 = Conv(2 * c_, c2, k=1, d=1, s=1)  # act=FReLU(c2)
            self.alphas_channel = None
        self.m = nn.Sequential(*[Bottleneck_search_merge(c_, c_, kd, candidate_e, shortcut,
                               g, e=1.0, gumbel_channel=gumbel_channel, separable=separable) for _ in range(n)])
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
    ...
    def forward(self, x):
        if self.search_c2:
            if self.gumbel_channel:
                alphas_channel = gumbel_softmax(F.log_softmax(
                    self.alphas_channel, dim=-1), hard=True)  # alphas_channel is one-hot vector
                out = self.cv3[0].forward_withAlpha(self.m(self.cv1.forward_withAlpha(
                    x, alphas_channel)), alphas_channel) + self.cv3[1].forward_withAlpha(self.cv2.forward_withAlpha(x, alphas_channel), alphas_channel)
                for idx, a in enumerate(alphas_channel):
                    if a > 0:
                        return self.cv3_act(self.cv3_bn[idx](out))
                raise (ValueError(
                    "If code runs here, then there must be something wrong!"))
            else:
                alphas_channel = nn.functional.softmax(
                    self.alphas_channel, dim=-1)
    ...
    def get_alphas(self):
        ...
        for m in self.m:
            alphas.extend(m.get_alphas())
        if self.search_c2:
            alphas.append(self.alphas_channel)
        return alphas
...
class Cells_search(nn.Module):
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, N=1, gumbel_op=False):
        ...
        for i in range(N):
            cell = Search_cell(steps, multiplier, C_prev_prev,
                               C_prev, C_curr, reduction, reduction_prev, gumbel_op)
        ...
        self.register_buffer('alphas', torch.autograd.Variable(
            1e-3*torch.randn(k, len(PRIMITIVES)), requires_grad=True))

    def forward(self, x):
        assert (len(x) == self.num_input)
        if self.gumbel_op:
            weights = gumbel_softmax(F.log_softmax(
                self.alphas, dim=-1), hard=True)
        else:
...
class Cells_search_merge(nn.Module):
    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, N=1, gumbel_op=False):
        ...
        self.num_input = 1 if C_prev_prev == None else 2
        for i in range(N):
            cell = Search_cell_merge(
                steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, gumbel_op)
            C_prev_prev, C_prev = C_prev, multiplier*C_curr
            if self.num_input == 1:
                C_prev_prev = None
            reduction_prev = reduction
...
class Cells(nn.Module):
    def __init__(self, genotype, concat, C_prev_prev, C_prev, C, reduction, reduction_prev, N=1, drop_path_prob=0.0):
        ...
        self.num_input = 1 if C_prev_prev == None else 2
        for i in range(N):
            cell = Cell(genotype, concat, C_prev_prev, C_prev,
...
class AFF(nn.Module):
    ...
    def __init__(self, c1s, c2, strides, kd=[(1, 1), (3, 1), (5, 1), (3, 2)], candidate_e=[1.], gumbel_channel=False, separable=False):
        ...
        for c, s in zip(c1s, strides):
            ...
            tmp_kd = kd
            if s < 1:
                self.m.append(my_conv(c, c2, kd=tmp_kd, candidate_e=candidate_e,
                              s=1, gumbel_channel=gumbel_channel, inside_alphas_channel=False))
                self.up_sample.append(nn.Upsample(
                    None, int(1./s), mode='nearest'))
            else:
                self.m.append(my_conv(c, c2, kd=tmp_kd, candidate_e=candidate_e,
                              s=s, gumbel_channel=gumbel_channel, inside_alphas_channel=False))
                self.up_sample.append(nn.Identity())
        self.register_buffer('alphas_edge', torch.autograd.Variable(
            1e-3*torch.randn(len(c1s)), requires_grad=True))  # architecture parameters for super-edges
        if len(self.candidate_e) > 1:
            self.register_buffer('alphas_channel', torch.autograd.Variable(
                1e-3*torch.randn(len(self.candidate_e)), requires_grad=True))
        else:
            self.alphas_channel = None
    ...
    def forward(self, xs):
        ...
        if self.alphas_channel is not None:
            ...
            for idx, (m, up, x) in enumerate(zip(self.m, self.up_sample, xs)):
                out += up(m.forward_withAlpha(x, alphas_channel)) * \
                    alphas_edge[idx]
        else:
            for idx, (m, up, x) in enumerate(zip(self.m, self.up_sample, xs)):
...
class SPP_search(nn.Module):
    ...
    def __init__(self, c1, c2, k=(5, 9, 13)):
        ...
        c_ = c1 // 2  # hidden channels
#        self.cv1 = Conv(c1, c_, k=1, d=1, s=1)
        self.cv1 = Conv_search_merge(
            c1, c_, kd=[(1, 1)], candidate_e=[1.], s=1)
        self.cv2 = Conv(c_ * (len(k) + 1), c2, k=1, d=1, s=1)
        self.m = nn.ModuleList(
            [nn.MaxPool2d(kernel_size=x, stride=1, padding=x // 2) for x in k])
...
class Classify(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):
        ...
        self.aap = nn.pAdaptiveAvgPool2d(1)  # to x(b,c1,1,1)
...
class Sequential(Backbone):
    ...
    def output_shape(self):
        ...
        return {
            module.id: ShapeSpec(
                channels=module.max_out_channels,
            )
            for module in self.layers if module.id
        }
